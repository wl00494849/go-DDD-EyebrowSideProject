...
func (r *AnnouncementRepo) GetNewDetail(newID string) *entity.Announcement {
	...
	row := r.db.QueryRow("Select Id,Title,Content,Create_Time from Announcement where Id=?", newID)
...
func (r *AnnouncementRepo) GetNewList() *[]entity.Announcement {
	...
	rows, _ := r.db.Query("Select Id,Title,Create_Time,IsTop from Announcement")
	defer rows.Close()
	for rows.Next() {
		var data entity.Announcement
		rows.Scan(&data.Id, &data.Title, &data.Create_Time, &data.IsTop)
		sli = append(sli, data)
	}
	return &sli
}
